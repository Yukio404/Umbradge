...
def traceroute():
    ...
    num_dest = input('IP Adress or Server Name: ')
    adr_dest = socket.gethostbyname(num_dest)
    print (adr_dest)
    ip_list = []
    ...
    for i, traced_ip in enumerate(traceroute(adr_dest, 33434)):
    		if(traced_ip != None and traced_ip not in ip_list):
    			...
    			response = ip_info(url)
    			if 'bogon' not in response:
    					print(str(i+1) + ')' + '\t' + traced_ip + '\t- ' + response['city'] + ', ' + response['region'] + ', ' + response['country'])
    			else:
